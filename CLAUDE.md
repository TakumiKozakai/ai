...
| `finalize-unit-case-normal-case` | `create-unit-case-normal-case`（未作成時のみ）→ サイクルA: `review-unit-case-normal-case` → 作成 skill の指摘反映モード、を収束まで最大5回 → サイクルB: `review-unit-case-normal-case-from-source` で同様に最大5回。分類ごとに独立した finalize（他分類は後日同じ型で追加） |

### 2系統の入力と成果物

「設計書ベース」と「ソースベース（`-from-source` 接尾辞）」は別 skill。作成 skill では入力系統によって成果物の形式が異なるが、レビュー skill はどちらも分類別4ファイルを対象にする。

- 設計書ベース: 試験分類別に4ファイル `試験項目票_{画面名称}_正常系.md` / `_01_フロントバリデーション.md` / `_02_サーババリデーション.md` / `_03_異常系.md`。出力先は既定で設計書と同じディレクトリ。
- ソースベースの作成（`create-unit-case-for-screen-from-source`）: 単一ファイル `05_単体試験/claude-playwrite-unit-test/試験項目票_{画面名称}.md`。
- ソースベースのレビュー（`review-unit-case-*-from-source`）: 設計書ベースと同じ分類別4ファイルが対象。照合元がソースになるだけで、成果物の形式は変わらない。
- レビュー結果 HTML: 単体実行では指摘が1件以上あるときだけ作成する（0件ならチャット報告のみ）。集約実行では作成しない。finalize は上限打ち切り時のみ作成する。

### 実行モードとサブエージェント

- **集約実行**: 親 skill から「集約実行」と明示して呼ばれた場合、子 skill は HTML を作らず、指摘一覧・実行状態・未確認観点を親に返す。明示が無ければ単体実行として振る舞う。正常系の子 skill はサブエージェントとして動くため（`AskUserQuestion` はサブエージェントで使えない）、単体実行でもユーザーに質問せず、確認したい内容を「要確認事項」「未確認観点」として返す。呼び出し元（finalize、または人と対話しているセッション）がユーザーに確認して「確認済み回答」として次の呼び出しに渡す。
...
- 他3分類の作成 skill の指摘反映モードは旧設計（指摘ID・原文・ファイル間移動を含む）のまま残っており、分類別 finalize を追加する際に正常系と同じ型へ簡素化する予定。
