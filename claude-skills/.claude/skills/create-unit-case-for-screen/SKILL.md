---
name: create-unit-case-for-screen
...
2. 以下4つのskillのSKILL.mdを順に読み込み、その手順に従って同じ設計書ファイルパス・出力先ディレクトリで実行する（`.claude/skills/`は、本skill自身が配置されている `claude-skills/` を基準に解決する）
   1. `.claude/skills/create-unit-case-normal-case/SKILL.md`（正常系）
   2. `.claude/skills/create-unit-case-front-validation/SKILL.md`（異常系-フロントバリデーション）
   3. `.claude/skills/create-unit-case-server-validation/SKILL.md`（異常系-サーババリデーション）
   4. `.claude/skills/create-unit-case-server-error/SKILL.md`（異常系-サーバエラー）
