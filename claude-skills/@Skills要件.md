...
| 単体試験項目作成 | 画面 | 正常系 | create-unit-case-normal-case | 設計書をもとに正常系項目を起票する（対象画面機能を指定すれば範囲を限定できる） |
...
| | | 入力チェック | | |
| | | 異常系 | | |
| 単体試験項目Rv | 画面 | 正常系（設計書） | review-unit-case-normal-case | 設計書をもとに正常系項目をRvする（対象画面機能を指定すれば範囲を限定できる） |
...
| | | 正常系（ソース） | review-unit-case-normal-case-from-source | ソースをもとに正常系項目をRvする（対象画面機能を指定すれば範囲を限定できる） |
...
| 単体試験項目完成 | 画面 | 正常系 | finalize-unit-case-normal-case | 正常系の試験票を未作成時に設計書から作成し、設計書Rv→ソースRvの順に、Rv→指摘反映を収束または各最大5回まで繰り返す。対象画面機能を指定すれば範囲を限定できる。作成・反映はSonnet、RvはOpusのサブエージェント |
...
- 引数（`$ARGUMENTS`。自由記述で渡す）
  ...
  - 対象画面機能（イベント。省略可。例:「画面初期表示」「登録ボタン押下」。省略時は画面全体が対象）
...
- 対象画面機能を指定すると、その機能に絞った試験項目のみを完成させる（内部で呼ぶ作成・レビューskillに引数として引き継ぐ）
...
- 対象画面機能を指定した場合、完成させるのはその機能分のみで画面全体の網羅性は保証しない。既存の試験項目票がある場合、指定機能に該当する行のみをマージ（追加・更新）し、他機能の既存行は保持する

### finalize-unit-case-front-validation

#### 使い方

- 引数・呼び出し方は `finalize-unit-case-normal-case` と同一（設計書ファイルパス・対象画面のソースファイルパス・出力先ディレクトリ）。ただし対象画面機能によるスコープ指定は未対応
...
- `$ARGUMENTS` は「通常モード」と「指摘反映モード」で内容が異なる
  - 通常モード: 設計書ファイルパス（必須）、出力先ディレクトリ（省略可）、対象画面機能（イベント。省略可。例:「画面初期表示」「登録ボタン押下」。省略時は画面全体が対象）
  - 指摘反映モード: 設計書ファイルパス、出力先ディレクトリ、指摘一覧（種別・対象No.・シナリオ・指摘内容・根拠）、照合元（設計書／ソース。ソースの場合はソースファイルパス）、確認済み回答（あれば）。指摘一覧と照合元の両方が含まれていれば指摘反映モードとして動作する（対象画面機能は指摘反映モードでは使用しない）
...
- 対象画面機能を指定した場合、設計書の記述内容（要素・操作フロー・エンドポイント等）と突き合わせて該当範囲を特定し、その機能分の項目のみを洗い出す。一意に特定できない場合は起票せず要確認事項として返す
  - 既存の試験項目票が無い場合は、指定機能分の行のみを含むファイルを新規作成する（画面全体を網羅しない旨を報告する）
  - 既存の試験項目票がある場合は、該当する既存行を更新（上書き可の明示が必要）または新規行として追加し、他機能の既存行は保持したうえで全体を再採番する（マージ）
...
- `create-unit-case-normal-case` と同じ引数構成（通常モード／指摘反映モード）。ただし対象画面機能によるスコープ指定は未対応
...
- 引数: 設計書ファイルパス（必須）、試験項目票ディレクトリ（省略可。省略時は設計書と同じディレクトリ）、対象画面機能（イベント。省略可。省略時は画面全体が対象）
- 大元skill（`finalize-unit-case-normal-case`）から呼ぶ場合は「集約実行」であることと画面名称・対象画面機能・確認済み回答・未解決事項を明示して渡す。明示が無ければ単体実行として振る舞う
...
- 対象画面機能を指定した場合、設計書と試験項目票の双方をその機能に該当する範囲に絞って照合する。一意に特定できない場合は「未確認」として返す
- 集約実行の場合はHTMLを作らず、指摘一覧・実行状態・未確認観点・対象範囲（画面全体、または対象画面機能名）（と本skill内で得た確認済み回答）を大元skillに返す。単体実行の場合は指摘が1件以上あれば `試験項目票レビュー結果_{画面名称}_正常系_設計書_{yyyyMMddHHmm}.html` を作成する
...
- 引数: 対象画面のソースファイルパス（複数可、必須）、試験項目票ディレクトリ（必須。省略時はユーザーに確認、集約実行では失敗を返す）、対象画面機能（イベント。省略可）
- それ以外は `review-unit-case-normal-case` と同様（集約実行の明示、確認済み回答・未解決事項の受け渡し、対象画面機能によるレビュー範囲の限定）

#### Skillの詳細

- 照合元が設計書ではなくソースコード（テンプレート・コントローラ・フォームクラス・サービス・リポジトリ等の関連ソース）になる点以外は `review-unit-case-normal-case` と同じ（対象画面機能を指定した場合の範囲限定を含む）
...
- `review-unit-case-normal-case` と同じ引数構成（設計書ファイルパス・試験項目票ディレクトリ）。ただし対象画面機能によるスコープ指定は未対応
- 担当ファイルは `試験項目票_{画面名称}_01_フロントバリデーション.md`
...
- `review-unit-case-normal-case-from-source` と同じ引数構成（ソースファイルパス・試験項目票ディレクトリ）。ただし対象画面機能によるスコープ指定は未対応
- 担当ファイルは `試験項目票_{画面名称}_01_フロントバリデーション.md`
